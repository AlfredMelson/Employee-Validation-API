...
const handleAdminRefreshToken = (req: Request, res: Response) => {
  ...
  const foundAdmin = adminDB.administrators.find(admin => admin.refreshToken === refreshToken)
  ...
  jwt.verify(
    ...
    (err: any, decoded: any) => {
      // send status 403: 'forbidden; no access rights to the content'; ie invalid token or the foundAdmin username is not the same as the decoded username
      ...
      // accesstoken is sent as json, that the FED can use to authenticate the admin.
